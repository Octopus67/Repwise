...
const getLandmarkStyles = (c: ThemeColors): { key: keyof WNSLandmarks; label: string; color: string; dash?: string }[] => [
  { key: 'mev', label: 'MEV', color: c.semantic.warning, dash: '4,3' },
  { key: 'mav_high', label: 'MAV', color: c.semantic.positive, dash: '4,3' },
  { key: 'mrv', label: 'MRV', color: c.semantic.negative, dash: '4,3' },
];
  ...
  return (
    ...
    >
      <Svg width="100%" height={CHART_H} viewBox={`0 0 ${CHART_W} ${CHART_H}`}>
        ...
        {getLandmarkStyles(c).map(({ key, label, color, dash }) => {
