...
def upgrade() -> None:
    ...
    op.execute("UPDATE progress_photos SET pose_type = 'front_relaxed' WHERE pose_type = 'front'")


def downgrade() -> None:
    # Revert 'front_relaxed' back to 'front'
    op.execute("UPDATE progress_photos SET pose_type = 'front' WHERE pose_type = 'front_relaxed'")
