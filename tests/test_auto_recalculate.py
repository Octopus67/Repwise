...
async def test_auto_recalculate_on_significant_weight_change(db_session):
    ...
    profile = UserProfile(user_id=user_id, preferences={"age_years": 30, "sex": "male"})
    db_session.add(profile)

    # Create user goal
    goal = UserGoal(user_id=user_id, goal_type="cutting", goal_rate_per_week=-0.5)
    db_session.add(goal)
    ...
    with patch("src.modules.user.service.compute_snapshot") as mock_compute:
        ...
        result = await service.log_bodyweight(
            user_id, BodyweightLogCreate(weight_kg=82.0, recorded_date=date.today())
        )
...
async def test_no_auto_recalculate_on_small_weight_change(db_session):
    ...
    with patch("src.modules.user.service.compute_snapshot") as mock_compute:
        ...
        result = await service.log_bodyweight(
            user_id, BodyweightLogCreate(weight_kg=80.5, recorded_date=date.today())
        )
